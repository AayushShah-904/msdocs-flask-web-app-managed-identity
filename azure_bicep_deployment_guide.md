...
## 📂 Bicep File and Module Structure

The deployment templates are structured modularly inside the `infra/` folder at the root of the project:

```
infra/
├── main.bicep              # Main orchestrator template (coordinates modules & role assignments)
├── main.bicepparam         # Parameter file containing location, suffixes, and passwords
├── main.json               # Compiled ARM JSON template generated by Bicep build
└── modules/
    ├── acr.bicep           # Provisions Azure Container Registry (ACR)
    ├── storage.bicep       # Provisions Storage Account and private 'photos' blob container
    ├── app.bicep           # Provisions Linux App Service Plan and Web App + app settings
    └── db.bicep            # Provisions PostgreSQL, Firewall, Database, and Entra AD Admin
```
